...
function NewsDetails({ eventData, newsItems }) {
        ...
        for (let item of newsItems) {
            if (item.activity && Array.isArray(item.rssContent)) {
                for (let news of item.rssContent) {
                    ...
                    const slugTitle = news.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
                    if (formattedDate === `${year}-${month}-${day}` && slugTitle === title) {
                        return {
                            newsDetails: news,
                            parentDetails: {
    ...
    const indusrtyEvents = (eventData || []).filter(event => {
        if (news && news.parentDetails && news.parentDetails.industry) {
            return event.industry.toLowerCase() === news.parentDetails.industry.toLowerCase();
        }
        return false;
    });
            ...
            <div className='news-detail-div'>
                <div className='news-details-header'>
                    <div className='news-details-title-details'>
                        <h1 className='news-details-title'>{selectedNews.newsDetails?.title || 'No Title Available'}</h1>
                        <div className='news-details-more-details'>
                            <div className='news-details-icon-name'>
                                <div className='news-details-icon'>
                                    <img src={selectedNews.parentDetails?.icon ? `https://snfyibackend.onrender.com/${selectedNews.parentDetails.icon.replace(/\\/g, "/").replace("uploads/", "")}` : ''} alt="" />
                                </div>
                                <span className='news-details-souce-name'>{selectedNews.parentDetails?.title || 'No Source Available'}</span>
                            </div>
                            <div className='news-details-dates'>
                                <span className='news-details-date'>{formatDate(selectedNews.newsDetails?.publishDate)}</span>
                            </div>
                            <div className='news-details-publisher'>
                                <span className='news-details-publisher-name'>{selectedNews.newsDetails?.author || 'Unknown Author'}</span>
                            </div>
                        </div>
                    </div>
                    <div className='news-details-banner'>
                        <img src={selectedNews.newsDetails?.imageUrl || ''} alt="News Image" />
                    </div>
                    <div className='news-details-dis'>
                        <span className='news-details-discription'>{extractTextFromPTags(selectedNews.newsDetails?.description)}</span>
                        <button className='news-details-button' onClick={() => handleReadMoreClick(selectedNews.newsDetails?.link)} >Read more</button>
                    </div>
                </div>
                {indusrtyEvents.slice(0, 5).length > 0 ? <div className='allnews-events'>
                    <h2 className='upcoming-event-text'>Upcoming Events</h2>
                    <div className='upcoming-event-cards'>
                        {indusrtyEvents ? indusrtyEvents.slice(0, 5).map((event, index) => (
            ...
            <div className='latest-news-container'>
                <div className='latest-news-details'>
                    ...
                    <div className='latest-news-cards'>
                        {similar ? similar.map((news, index) => (
                ...
                <div className='company-details-container'>
                    ...
                    <button className='company-button' onClick={handleClick} >Join our Community</button>
                </div>
            </div>
            {/* National Events */}
            <div className='national-events'>
                <div className='national-events-details'>
                    ...
                    <div className='national-events-cards'>
                        {nationalEvents.length > 0 ? nationalEvents.slice(0, 4).map((event, index) => (
