...
function NewsDisplay({ newsItems, loading, setLoading, filteredEventData }) {
    ...
    function getLatestNewsone(newsItems) {
        ...
        allNews.sort((a, b) => new Date(b.newsDetails.publishDate) - new Date(a.newsDetails.publishDate));
    ...
    const handleCardClick = () => {
        ...
        navigate(`/${formattedDate}/${newsTitleSlug}`, { state: { news: latestNews, similar: latest.slice(1, 5), filteredEventData } });
    };
    ...
    return (
        <>
            {loading || !latest[0] || <div className='card-one-container' onClick={handleCardClick}>
                <div className='card-one-image'>
                    <img src={latest[0].newsDetails.imageUrl} alt='No Image is available to Display' />
                </div>
                <div className='card-one-details'>
                    <div className='card-one-subdetails'>
                        <span className='card-one-industry'>{capitalizeIndustry(latest[0].parentDetails.industry)}</span>
                        <h2 className='card-one-title'>{latest[0].newsDetails.title}</h2>
                        <p className='card-one-description'>{extractTextFromPTags(latest[0].newsDetails.description)}</p>
                    </div>
                    <div className='card-one-subdetilstwo'>
                        <div className='card-one-icons'>
                            <div className='card-three-icon'>
                                <img src={latest[0].parentDetails.icon ? `https://snfyibackend.onrender.com/${latest[0].parentDetails.icon.replace(/\\/g, "/").replace("uploads/", "")}` : ''} alt="News Icon" />
                            </div>
                            <span className='card-one-source-name'>{latest[0].parentDetails.title}</span>
                            <span className='card-one-source-name'>{timeAgoInHours(latest[0].newsDetails.publishDate)}</span>
                        </div>
