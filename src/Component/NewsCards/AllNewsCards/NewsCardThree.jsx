...
function NewsCardThree({ news, allNewsData, filteredEventData }) {
    ...
    const handleCardClick = () => {
        ...
        navigate(`/${formattedDate}/${newsTitleSlug}`, { state: { news: news, similar: allNewsData.slice(1, 5), filteredEventData } });
    };
    ...
    return (
        <div className='card-three-container' onClick={handleCardClick}>
            <div className='card-three-image'>
                <img src={news.newsDetails.imageUrl} 
                 alt='No Image is available to Display' />
            </div>
            <div className='card-three-details'>
                <div className='card-three-subdetails'>
                    <span className='card-three-industry'>{capitalizeIndustry(news.parentDetails.industry)}</span>
                    <h2 className='card-three-title'>{truncateText(news.newsDetails.title)}</h2>
                    <p className='card-three-description'>{extractTextFromPTags(news.newsDetails.description)}</p>
                </div>
                <div className='news-card-four-icons'>
                    <div className='news-card-four-icons-name'>
                        ...
                        <span className='card-three-source-name' > {news.parentDetails.title}</span>
                    </div>
                    <span className='news-card-three-time' >{news.newsDetails.publishDate ? convertTimeToRelativeTime(news.newsDetails.publishDate) : 'No date'}</span>
                </div>
            </div>
        </div>
    );
