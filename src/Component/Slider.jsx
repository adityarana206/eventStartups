            ...
            <div className='slider-images' >
                ...
                >
                    {slides.slice(0, 12).map((image, index) => {
                        ...
                        const imageUrl = image ? `http://startupnewsfyi-env.eba-msvvew3i.ap-south-1.elasticbeanstalk.com/${image.replace(/\\/g, "/").replace("uploads/", "")}` : '';
                        return (
                            <div key={index} className={getSlideClass(index)}>
                                <img src={imageUrl} onClick={() => handleCardClick(event._id)} alt={`Slide ${index}`} />
                            </div>
                        );
                    })}
                </div>
            ...
            <div className="dots">
                {images.map((_, index) => (
                    <span
                        ...
                        onClick={() => { handleDotClick(index); setRight(prev => prev + 3) }}
                    ></span>
                ))}
            </div>
        </div>
