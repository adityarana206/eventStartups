...
const EventCard = ({ event }) => {
    ...
    const handleCardClick = (eventname) => {
        ...
        if (!recentlyViewed.includes(eventname)) {
            ...
            sessionStorage.setItem('recentlyViewedEvents', JSON.stringify(recentlyViewed));
        }
    ...
    function extractLocation(fullAddress) {
        ...
        if (parts.length > 2) {
            ...
            const location = parts.slice(0, 2).map(part => part.trim()).join(', ');
            return location;
        } else {
    ...
    const imageUrl = event.image ? `https://snfyibackend.onrender.com/${event.image.replace(/\\/g, "/").replace("uploads/", "")}` : '';
    return (
        <div className="event-card" onClick={() => handleCardClick(event.eventname)}>
            ...
            <div className="event-content">
                ...
                <div className="event-details-card">
                    ...
                    <div className="event-detail">
                        <div><LocationIcon className="event-icon" /></div>
                        <span className='event-card-location' >{limitWords(event.location, 5)}</span>
                    </div>
                </div>
                <div className="event-footer">
                    <span className="event-price">{event?.tickets[0]?.priceWithTax !== 0 ? "₹" + event?.tickets[0]?.priceWithTax : "N/A"}</span>
                    <button className="register-button" onClick={() => handleCardClick(event.eventname)} >Register Now</button>
                </div>
            </div>
        </div>
    );
