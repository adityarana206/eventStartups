...
function News() {
                ...
                try {
                    ...
                    const fetchedItems = rssResponse.data.items.map(item => {
                        ...
                        if (imageUrl && imageUrl.includes('cdn-cgi/image')) {
                            ...
                            imageUrl = imageUrl.replace(/width=\d+/g, 'width=1200').replace(/height=\d+/g, 'height=800'); // Set larger size
                        }
    ...
    useEffect(() => {
        ...
        loadNewsData();
        const interval = setInterval(loadNewsData, 120000);
    ...
    const filteredNewsItems = selectedIndustry === 'All'
        ? newsItems
        : newsItems.filter(newsItem => newsItem.industry === selectedIndustry.toLocaleLowerCase());

    const filteredEventData = selectedIndustry === 'All'
        ? eventData
        : eventData.filter(event => event.industry.toLowerCase() === selectedIndustry.toLowerCase());
    ...
    return (
        <div className='news-container'>
            ...
            {!showLatestNews ? (
                ...
                <LatestNews newsItems={newsItems} filteredEventData={filteredEventData}
                    eventData={eventData}
                    selectedIndustry={selectedIndustry}
                    loading={loading}
                    setLoading={setLoading} />
