...
const FilterOnEvent = ({
  ...
  return (
    <div className="filter-container-event">
      <span style={{ fontSize: "14px", fontWeight: "500", color: "#c6c6c6" }}>
        Time:
      </span>
        ...
        {filters.map((filter) => (
          <button
            ...
            onClick={() => {
              ...
              onFilterChange(filter.value, eventData);
            }}
          >
            {filter.label}
          </button>
  ...
  useEffect(() => {
    if (activeButtonRef.current) {
      ...
      const scrollPosition =
        activeButtonPosition - containerWidth / 2 + buttonWidth / 2;
...
function Events({ setSelectedLocation, isPopupOpen, setIsPopupOpen }) {
  const [eventData, setEventData] = useState(mockEventsList);
  const [filteredEventData, setFilteredEventData] = useState(mockEventsList);
  ...
  useEffect(() => {
    // fetchEventData(); // Disabled to use mock data instantly on startup
  }, []);
      ...
      if (!hasAppliedInitialFilters.current) {
        ...
        if (location.state?.industry) {
          filtered = filtered.filter(
            (event) =>
              event.industry === location.state.industry ||
              event.industry === "All",
          );
          setActiveIndustry(location.state.industry);
          setShowVariableIndustry(location.state.industry);
        ...
        if (activeIndustry) {
          const industrySpecificEvents = filtered.filter(
            (event) => event.industry === activeIndustry,
          );
          const allCategoryEvents = filtered.filter(
            (event) => event.industry === "All",
          );
          // Combine industry-specific events first, then "All" events
          filtered = [...industrySpecificEvents, ...allCategoryEvents];
        }
    ...
    switch (filter) {
      ...
      case "this-weekend":
        ...
        weekendStart.setDate(
          weekendStart.getDate() + (5 - weekendStart.getDay()),
        );
  ...
  const onFilterChange = (filter) => {
    ...
    if (selectedLocation) {
      filtered = filtered.filter(
        (event) => event.city.toLowerCase() === selectedLocation.toLowerCase(),
      );
    }
    ...
    if (activeIndustry) {
      filtered = filtered.filter(
        (event) =>
          event.industry === activeIndustry || event.industry === "All",
      );
    }
  ...
  const handleSelectLocation = (location) => {
    ...
    const filteredEvents = eventData.filter(
      (event) => event.city.toLowerCase() === location.city.toLowerCase(),
    );
  ...
  const uniqueLocations = [
    ...new Set(eventData.map((event) => event.city)),
  ].map((city) => ({ city }));
    ...
    <div className="event-container">
      ...
      {loading ? (
        ...
        <div className="show-event-card">
          <h2 className="to-show-all-events">
            All {convertToTitleCase(showVariableName)}{" "}
            {convertToTitleCase(showVariableIndustry)} Events
          </h2>
