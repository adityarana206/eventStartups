...
const AttendeeDetails = () => {
    ...
    const { selectedTickets, orderSummary, organisationInfo, ticketPrices, couponCode, validCouponCodes, eventData, couponValue, eventId, ticketInfo, eventName } = location.state;
    ...
    useEffect(() => {
        ...
        setPgcharges(((totalAfterDiscound + totalAfterDiscound * 0.18) * 0.025).toFixed(2))
    ...
    const handleSubmit = async (updatedAttendees) => {
        ...
        try {
            if (eventId) {
                const eventResponse = await axios.put(`http://localhost:5001/updateEvent-by-user/${eventId}`, eventDetails, {
                    headers: {
                ...
                const eventResponse = await axios.post('http://localhost:5001/registerEvent', eventDetails, {
                    headers: {
                        'Content-Type': 'application/json'
                    }
                });
    ...
    const sendConfirmationEmail = async (attendee, eventData, organisationInfo) => {
...
9625952588`
        ...
        try {
            const response = await axios.post('http://localhost:5001/send-email', emailInfo);
    ...
    const sendConfirmationEmailForPayment = async (attendee, eventData, Amount) => {
        const date = new Date();
...
startupnewsfyi@gmail.com`
    ...
    const getSessionId = async (details) => {
        ...
        try {
            let res = await axios.post("http://localhost:5001/payment", details);
    ...
    const handleClick = async (e) => {
        ...
        try {
            let sessionId = await getSessionId(details);
            if (!sessionId) {
                alert('Failed to initialize payment session.');
                return; // Stop the process if no session ID was retrieved
            }
            let checkoutOptions = {
            ...
            cashfree.checkout(checkoutOptions).then(async (res) => {
                ...
                if (res.error) {
                    ...
                    if (updatedAttendees) {
                        ...
                        //  SendEmail(updatedAttendees, eventData); // Send emails based on the latest state
                    }
                    return;
                }
                else if (res.redirect) {
                    alert('Payment successful redirect')
                    const updatedAttendees = await updateAttendeesStatus("Successful", order_id);
                    if (updatedAttendees) {
                        handleSubmit(updatedAttendees); // Pass the updated attendees directly
    ...
    const availableTicketTypes = (index) => {
        ...
        return Object.keys(ticketCounts).filter((ticket) => ticketCounts[ticket] > 0);
    };
    ...
    const isAttendeeValid = (attendee, index) => {
        ...
        const requiredFields = ['name', 'email', 'phone', 'city', 'company', 'industry', 'designation', 'ticketType'];

        const allFieldsFilled = requiredFields.every(field => attendee[field].trim() !== '');
    ...
    return (
        <>
            <form className='attendee-form' >
                    ...
                    <div className='tickets-details' >
                        <div className='priceandaddmore'>
                            <div className='priceandtax' >
                                <div className='priceandtotal' >
                                    ...
                                    {Object.entries(ticketSummary).map(([ticketType, { quantity, price }]) => (
                                ...
                                <div className='ticketPrice' >
                                    ...
                                    <div onClick={handleDeleteAndRedirect} style={{ cursor: 'pointer' }} ><DeleteIcon /></div>
                                </div>
                            </div>
                            <div className='addmorecontainer' >
                                <div className='addmore' onClick={eventDetails}>
                                    <span className='addmoretext' >Buy more</span>
                                    <span onClick={eventDetails}><AddMoreIcon /></span>
                                </div>
                            </div>
                        </div>
                        <div className='coupeninputandcode'>
                            ...
                            <div className="coupon-container">
                                ...
                                />
                                <button type="button" className='apply-button' onClick={applyCoupon}>Apply</button>
                            </div>
                        ...
                        <div className='payment-details' >
                            <div className='payment-small-details' >
                                ...
                                <div className='payment-subdetails' >
                                    <span className='amount-text' >Ticket Amount {ticket.snfyiPayer === "buyer" ? "(Base Price)" : "(Inc. of taxes)"}</span>
                                    <span className='amont-value'>₹{totalTicketPrice}</span>
                                </div>
                                {discount > 0 && <div className='payment-subdetails'>
                                    <span className='amount-text'>Discount</span>
                                    <span className='amont-value'>₹{discount}</span>
                                </div>}
                                {ticket.snfyiPayer === "buyer" && <div className='payment-subdetails'>
                                ...
                                <div className='payment-subdetails'>
                                    ...
                                    <span className='amont-value strikethrough'>₹{ticket.snfyiPayer === "buyer" ? ` ${pgchares} ` : `${totalTax}`}</span>
                                </div>
                            </div>
                            <div className='totalprice' >
                                <div className='payment-subdetails'>
                                    <span className='total-price-text' >Total Amount:</span>
                                    <span className='total-price-value' >₹{calculateTotalPrice().toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                        <button className='sava-button-submit-order' onClick={handleSaveAndNextAccordion} type='button' >CONTINUE</button>
                    </div>
                </Accordion>
                        ...
                        <div className='attendee-details'>
                            ...
                            <div className='emailTicketType' >
                                <div className='email-input-error' >
                                    ...
                                    {emailErrors[index] && <p className="email-error-message">{emailErrors[index]}</p>}
                                </div>
                                <select
                                    name="ticketType"
                                    ...
                                    required
                                >
                                    <option className='option' value="">Select Ticket Type</option>
                            ...
                            <div className='emailTicketType'>
                                <div className='email-input-error'>
                                    ...
                                    {phoneErrors[index] && <p className="email-error-message">{phoneErrors[index]}</p>}
                                </div>
                                <div className='input-text-box' >
                                    <input
                                        name="city"
                            ...
                            <button type="button" className='sava-button' disabled={!isAttendeeValid(attendee, index)} onClick={handleSaveAndNextAccordion}>
                                CONTINUE
                            </button>
                        </div>
                    </Accordion>
                ...
                {openAccordionIndex === attendees.length + 1 && <button onClick={handleClick} className='sava-button-submit' >  CONTINUE  </button>}
            </form >
